...
def macro():
	...
	dialog.addTopWindowListener(TopWindowListener(dirpath, "dialog_topwindowlistener"))
	textlistener = TextListener(dirpath, "currencyfield_textlistener")
	...
	addControl("FixedText", {"PositionX": 106, "PositionY": 18, "Width": 100, "Height": 8, "Label": "My Label", "NoLabel": True}, {"addMouseListener": MouseListener(dirpath, "fixedtext_mouselistener", ctx, smgr)})  # , "Step": 0
	...
	addControl("Edit", {"PositionX": 106, "PositionY": 72, "Width": 60, "Height": 12, "Text": "MyText", "EchoChar": ord("*"), "HelpText": "EchoChar will be canceled when moving the focus with the tab key."}, {"addFocusListener": FocusListener(dirpath, "edit_focuslistener"), "addKeyListener": KeyListener(dirpath, "edit_keylistener", dialog)})
	addControl("FixedLine", {"PositionX": 106, "PositionY": 58, "Width": 100, "Height": 8, "Orientation": 0, "Label": "My FixedLine"})
	...
	addControl("DateField", {"PositionX": 166, "PositionY": 96, "Width": 55, "Height": 12, "Dropdown": True, "DateFormat": 9, "DateMin": dmin.Date, "DateMax": dmax.Date, "Date": d.Date, "Spin": True, "HelpText": "Min: {} Max: {}".format(dmin, dmax)}, {"addSpinListener": SpinListener(dirpath, "datefield_spinlistener")})	 # com.sun.star.util.Dateで日付を指定。
	...
	addControl("CheckBox", {"PositionX": 106, "PositionY": 168, "Width": 150, "Height": 8, "Label": "~Enable Close dialog Button", "TriState": True, "State": 1}, {"addItemListener": ItemListener(dirpath, "checkbox_itemlistener",dialog)})
	...
	addControl("ComboBox", {"PositionX": 160, "PositionY": 230, "Width": 60, "Height": 12, "Dropdown": True, "MaxTextLen": 10, "ReadOnly": False, "Autocomplete": True, "StringItemList": ("First Entry", "Second Entry", "Third Entry", "Fourth Entry")})  # 選択した文字列が取得できない。
	...
	addControl("FormattedField", {"PositionX": 106, "PositionY": 270, "Width": 100, "Height": 12, "EffectiveValue": 12348, "StrictFormat": True, "Spin": True, "FormatsSupplier": numberformatssupplier, "FormatKey": formatkey}, {"addSpinListener": SpinListener(dirpath, "formattedfield_spinlistener")})
	addControl("ScrollBar", {"PositionX": 230, "PositionY": 230, "Width": 8, "Height": 52, "Orientation": VERTICAL, "ScrollValueMin": 0, "ScrollValueMax": 100, "ScrollValue": 5, "LineIncrement": 2, "BlockIncrement": 10}, {"addAdjustmentListener": AdjustmentListener(dirpath, "scrollbar_adjustmentlistener", ctx, smgr, docframe)})
...
class MouseListener(unohelper.Base, XMouseListener):  # Editコントロールではうまく動かない。
	def __init__(self, dirpath, name, ctx, smgr):
	...
	def _createLog(self, mouseevent, methodname):
		...
		createLog(dirpath, filename, "Buttons: {}, ClickCount: {}, PopupTrigger: {}, Modifiers: {}\nX: {}, Y: {}\nSource: {}".format(mouseevent.Buttons, clickcount, mouseevent.PopupTrigger, mouseevent.Modifiers, mouseevent.X, mouseevent.Y, mouseevent.Source))	
class TextListener(unohelper.Base, XTextListener):
	...
	def textChanged(self, textevent):  # 複数回呼ばれるので前値との比較が必要。
		...
		dirpath, name = self.args
		filename = "_".join((name, inspect.currentframe().f_code.co_name))
		createLog(dirpath, filename, "Source: {}".format(textevent.Source))		
	def disposing(self, eventobject):
		dirpath, name = self.args
		filename = "_".join((name, inspect.currentframe().f_code.co_name))
		createLog(dirpath, filename, "Source: {}".format(eventobject.Source))			
class FocusListener(unohelper.Base, XFocusListener):
	...
	def focusGained(self, focusevent):
		...
		dirpath, name = self.args
		filename = "_".join((name, inspect.currentframe().f_code.co_name))
		createLog(dirpath, filename, "Source: {}".format(focusevent.Source))				
...
class KeyListener(unohelper.Base, XKeyListener):
	def __init__(self, dirpath, name, dialog):
...
# 	@enableRemoteDebugging
	def keyPressed(self, keyevent):
		...
		dirpath, name = self.args
		filename = "_".join((name, inspect.currentframe().f_code.co_name))
		createLog(dirpath, filename, "Source: {}".format(keyevent.Source))	
...
class SpinListener(unohelper.Base, XSpinListener):
	def __init__(self, dirpath, name):
		self.args = dirpath, name	
	def up(self, dirpath, name, spinevent):
		...
		dirpath, name = self.args
		filename = "_".join((name, inspect.currentframe().f_code.co_name))
		createLog(dirpath, filename, "Source: {}".format(spinevent.Source))			
...
# 	@enableRemoteDebugging
	def itemStateChanged(self, itemevent):
		...
		dirpath, name = self.args
		filename = "_".join((name, inspect.currentframe().f_code.co_name))
		createLog(dirpath, filename, "Selected: {}, Highlighted: {}, ItemId: {}\nSource: {}".format(itemevent.Selected, itemevent.Highlighted, itemevent.ItemId, itemevent.Source))	
	def disposing(self, eventobject):
		dirpath, name = self.args
		filename = "_".join((name, inspect.currentframe().f_code.co_name))
		createLog(dirpath, filename, "Source: {}".format(eventobject.Source))	
class AdjustmentListener(unohelper.Base, XAdjustmentListener):	# ブレークするとマウスのクリックが無効になる。
	def __init__(self, dirpath, name, ctx, smgr, parentframe):
		self.ctx = ctx
		self.smgr = smgr
	...
	def adjustmentValueChanged(self, adjustmentevent):  # 子ダイアログを表示させると2回呼ばれてしまう。
		...
		dirpath, name = self.args
		filename = "_".join((name, inspect.currentframe().f_code.co_name))
		createLog(dirpath, filename, "Source: {}".format(adjustmentevent.Source))			
	def disposing(self, eventobject):
		dirpath, name = self.args
		filename = "_".join((name, inspect.currentframe().f_code.co_name))
		createLog(dirpath, filename, "Source: {}".format(eventobject.Source))	
class CloseListener2(unohelper.Base, XCloseListener):  # コントロールダイアログのフレームに付ける。
	def __init__(self, dirpath, name):
		self.args = dirpath, name
