...
# 	frame.addCloseListener(CloseListener(args))  # CloseListener
	...
	menulistener.args = popupmenu, subpopupmenu, menulistener
	dialog.execute()
	dialog.dispose()
...
# 		createLog(menuevent.Source, "{}_{}".format(self.__class__.__name__, inspect.currentframe().f_code.co_name), "Source: {}".format(menuevent.Source))
		source = menuevent.Source
		cmd = source.getCommand(menuevent.MenuId)
		if cmd == "close":
			if self.frame is None:  # フレームがないときはモダルダイアログ。
				popupmenu, subpopupmenu, menulistener = self.args
				popupmenu.removeMenuListener(menulistener)
				subpopupmenu.removeMenuListener(menulistener)
